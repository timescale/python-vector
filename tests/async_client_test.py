...
from timescale_vector.client import Async, IvfflatIndex, HNSWIndex, DiskAnnIndex, SEARCH_RESULT_METADATA_IDX, \
    Predicates, UUIDTimeRange, DiskAnnIndexParams, uuid_from_time
from dotenv import load_dotenv, find_dotenv
import os


@pytest.mark.asyncio
async def test_vector() -> None:
    ...
    for schema in ["tschema", None]:
        ...
        rec = await vec.search([1.0, 2.0], limit=4,
                               filter=[{"key_1": "val_1"}, {"key2": "val2"}, {"no such key": "no such val"}])
        assert len(rec) == 2
        ...
        assert len(rec) == 2
        rec = await vec.search([1.0, 2.0], limit=4, predicates=Predicates("key_10", "<", 100) & (
                    Predicates("key", "==", "val2", ) | Predicates("key_2", "==", "val_2")))
        assert len(rec) == 1
        rec = await vec.search([1.0, 2.0], limit=4, predicates=Predicates("key_10", "<", 100) and (
                    Predicates("key", "==", "val2") or Predicates("key_2", "==", "val_2")))
        assert len(rec) == 1
        rec = await vec.search([1.0, 2.0], limit=4,
        ...
        rec = await vec.search([1.0, 2.0], limit=4, uuid_time_filter=UUIDTimeRange(start_date=specific_datetime,
        ...
        assert len(rec) == 0
        rec = await vec.search([1.0, 2.0], limit=4,
                               uuid_time_filter=UUIDTimeRange(end_date=specific_datetime + timedelta(seconds=1),
                                                              time_delta=timedelta(days=7)))
        assert len(rec) == 1
