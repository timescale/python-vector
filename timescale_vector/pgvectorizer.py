...
class Vectorize:
    ...
    def register(self) -> None:
        with psycopg2.connect(self.service_url) as conn, conn.cursor() as cursor:
            ...
            cursor.execute(f"""
                ...
                CREATE OR REPLACE FUNCTION {self.schema_name}.{self.trigger_name_fn}()
                ...
                INSERT INTO {self.schema_name}.{self.work_queue_table_name} SELECT {self.id_column_name}
    ...
    def process(
        ...
        with (
            psycopg2.connect(self.service_url) as conn,
            ...
            cursor.execute(f"""
                ...
                FROM locked_items
                LEFT JOIN {self.schema_name}.{self.table_name}
                ON {self.table_name}.{self.id_column_name} = locked_items.id
                WHERE locked = true
